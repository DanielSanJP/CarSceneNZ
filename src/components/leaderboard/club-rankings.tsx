...
import type { ClubRanking } from "@/types/leaderboard";

interface ClubRankingsProps {
  data: ClubRanking[];
}

export const ClubRankings = memo(function ClubRankings({
  data,
}: ClubRankingsProps) {
  ...
  return (
    <div className="space-y-4">
      <div className="text-center mb-6">
        <h2 className="text-2xl font-bold mb-2">🏆 Top Clubs</h2>
        <p className="text-muted-foreground">
          Ranked by total likes across all member cars
        </p>
      </div>
      ...
      {data.length === 0 ? (
        <Card className="text-center py-12">
          <CardContent>
            ...
            <h3 className="text-lg font-medium mb-2">No club rankings yet</h3>
            <p className="text-muted-foreground">
              Join or create a club to compete!
            </p>
          </CardContent>
        </Card>
      ) : (
